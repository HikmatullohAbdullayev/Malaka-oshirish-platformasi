...
function LoginForm(props) {
  ...
  return (
    ...
    >
      ...
      {errors.passport && (
        <p className="text-red-500 font-semibold ">{errors.passport.message}</p>
      )}

      <div className="relative">
        <label className="text-[18px] font-normal text-white" htmlFor="parol">
          Parol
        </label>
        <Input
          id="parol"
          ...
          {...register('parol', { required: 'JSHSHIR kiritish majburiy' })}
          placeholder="JSHSHIR"
        />
        <span onClick={changeHidden} className="absolute top-[50%] right-2">
          {' '}
          {hidden ? <VisibilityIcon /> : <VisibilityOffIcon />}
        </span>
      </div>
      {errors.parol && (
        <p className="text-red-500 font-semibold ">{errors.parol.message}</p>
      )}
